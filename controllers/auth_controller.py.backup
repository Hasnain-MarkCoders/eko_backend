...
class AuthController:
    ...
    async def firebase_auth(self, access_token: str, name: str = None, provider: str = None):
        ...
        try:
            ...
            decoded_token = await self.admin.auth.verify_id_token(access_token)
            ...
            user_data = await self.admin.auth.get_user(uid)
            user_data = user_data.__dict__
            
            # Check if user exists in database
            existing_user = await users.find_one({
                "email": user_data.get("email") or user_data.get("provider_data", [{}])[0].get("email")
            })
            ...
            if not existing_user:
                ...
                new_user = {
                    "uid": uid,
                    "email": user_data.get("email") or user_data.get("provider_data", [{}])[0].get("email"),
                    "name": name or user_data.get("display_name"),
                    "provider": provider or user_data.get("provider_data", [{}])[0].get("provider_id"),
                    "status": "active",
                    "welcome": True,
                    "image": user_data.get("provider_data", [{}])[0].get("photo_url") or default_photo,
                    "type": "user",
                    "notificationToken": "",
                    "isDeleted": False,
                ...
                # User exists, check if brand type
                ...
                # Generate JWT token
                token = jwt.encode({"_id": str(existing_user["_id"])}, TOKEN_KEY, algorithm="HS256")
